...
test('Validate existence of PayId in the CLP Payments Database', async ({ request }) => {
    const response = await request.post('/validate-payid', {
        data: { payId: '123456' }
    });
    const data = await response.json();
    expect(data).toEqual({ exists: false, message: 'PayId not found' });
});
...
test('Validate appropriate message when PayId exists', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toEqual({ exists: true, message: 'PayId exists' });
});
...
test('Check behavior with a null PayId', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toEqual({ exists: false, message: 'Invalid PayId' });
});
...
test('Validate PayId format enforcement during validation', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toEqual({ exists: false, message: 'Invalid PayId format' });
});